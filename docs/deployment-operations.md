...
npm run release:log-apm-redaction
...
`GET /operations/capacity-planning`과 시스템 설정 `12개월 용량 계획`은 실제 DB aggregate count와 첨부 byte를 baseline으로 사용해 월별 capacity forecast API와 화면 리포트를 제공한다. staging/production 배포 전 `CAPACITY_*` 한도·증가율을 provider quota와 최근 3개월 실측값으로 설정하고, 첫 경고/위험 월과 `docs/capacity-planning.md` 검토 기록을 배포 증적에 포함한다.
...
`npm run release:synthetic-monitor`는 `SYNTHETIC_MONITOR_API_BASE_URL`, `SYNTHETIC_MONITOR_EMAIL`, `SYNTHETIC_MONITOR_PASSWORD`로 로그인부터 결제 요청/승인/예산/거래처/보고서/지급 전 목록까지 읽기 전용 synthetic path를 점검한다. 운영 scheduler에서는 `SYNTHETIC_MONITOR_REQUIRE_CONFIG=true`, `SYNTHETIC_MONITOR_MAX_LATENCY_MS`, `SYNTHETIC_MONITOR_OUTPUT`을 설정하고, 실패 row의 requestId와 output JSON을 monitoring alert 및 go-live evidence에 보관한다.
...
정기 릴리즈는 `docs/maintenance-release-backlog-policy.md`의 월간 calendar, scope freeze, RC/staging/go-no-go, production freeze를 따르고, emergency hotfix와 운영 개선 요청은 같은 문서의 dual approval, rollback, post-deploy monitoring, GitHub intake/backlog workflow로 관리한다.
...
`npm run release:log-apm-redaction`는 `createSafeLoggerOptions`와 `sanitizeLogValue`가 운영 로그와 APM trace 형태 payload에서 secret, cookie, authorization, CSRF token, checksum, 계좌번호, signed file URL query를 제거하는지 fixture로 검증한다. 실제 외부 APM 수집기 화면 증적은 `PRODUCTION_ENVIRONMENT_INVENTORY_PATH` strict 검증의 APM masking evidence로 보관한다.
...
`GET /api/operations/audit-integrity-report`는 월 단위 감사 로그를 `createdAt asc, id asc`로 정렬해 `sha256` hash chain을 생성하고 tail hash를 반환한다. 외부 WORM 또는 감사 저장소를 쓰는 환경은 `AUDIT_ARCHIVE_MODE`, `AUDIT_ARCHIVE_ENDPOINT`, `AUDIT_ARCHIVE_BUCKET`, `AUDIT_ARCHIVE_EVIDENCE` 중 실제 연계 증적을 설정하고, 월마감/외부 감사 제출 시 tail hash와 보관소 영수증을 함께 보관한다.
...
4. staging API 서버를 배포하고 `/api/health`, `/api/health/version`, `/api/health/db`, `/api/health/storage`, `/api/health/file-security`, `/api/health/jobs`, `/api/health/integrations`, `/api/operations/report-jobs`, `/api/operations/data-quality`, `/api/operations/financial-reconciliation`, `/api/operations/financial-control-report`, `/api/operations/audit-integrity-report`를 확인한다.
...
10. 운영 monitor 또는 scheduler가 `npm run release:synthetic-monitor`를 5분 또는 10분 주기로 실행해 로그인, 결제 요청, 승인, 예산, 거래처, 보고서, 지급 전 목록, 운영 상태를 읽기 전용으로 점검하고, `POST /api/operations/report-jobs/run`, `POST /api/operations/business-failure-alerts/notify`, `POST /api/operations/financial-reconciliation/notify`를 주기적으로 호출해 예약 보고서 처리, retry/dead-letter 기록, 담당자 알림이 생성되는지 확인한다.
...
- `GET /api/operations/privacy-access-report`에서 개인정보 처리 inventory, 거래처 계좌 암호화/마스킹, 파일 다운로드 사유 누락, 외부 감사 read-only 접근 이력을 확인하고 원문 개인정보/계좌/signed URL token이 리포트에 포함되지 않는지 검증한다.
...
- backend 운영 로그와 APM trace payload는 `createSafeLoggerOptions`, `sanitizeLogValue`, `npm run release:log-apm-redaction`으로 signed file URL query, cookie, authorization, CSRF token, credential/secret/checksum/token 필드, 계좌번호 패턴을 마스킹한다. 외부 APM/로그 수집기 화면 증적은 production environment inventory에 보관한다.
