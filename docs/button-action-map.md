...
| 파일 | 업로드, 진행률 표시, 실패 재시도, 이탈 복구 metadata, 완료, 다운로드, 삭제 | `presignFileUpload`, `uploadFileContent(progress)`, `completeFileUpload`, `getFileDownload(reason)`, `deleteFile` | `/files/*` | `Attachment`, object storage adapter, scan status, 다운로드 사유 `AuditLog(download_request)`, 업로드 recovery metadata, 감사/보안 이벤트 |
