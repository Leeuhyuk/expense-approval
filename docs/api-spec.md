...
| `GET` | `/operations/audit-integrity-report` | 월 단위 감사 로그 hash chain, tail hash, 외부 보관소 연계 상태 리포트 생성 |
...
`/operations/audit-integrity-report`는 `system:manage` 또는 `audit:read` 권한이 필요하며, 월 단위 `AuditLog`를 `createdAt asc, id asc` 순서로 정렬해 `sha256` payload hash chain을 생성한다. 응답은 `payloadHash`, `previousHash`, `recordHash`, head/tail hash, checkpoint, `AUDIT_ARCHIVE_ENDPOINT`/`AUDIT_ARCHIVE_BUCKET`/`AUDIT_ARCHIVE_EVIDENCE` 기반 외부 보관소 연계 상태를 반환하고 `beforeValue`/`afterValue` 원문 JSON은 포함하지 않는다.
...
감사 로그는 append-only 원칙을 따른다. 삭제 API를 제공하지 않고, release gate와 DB trigger로 수정/삭제를 차단한다. 기본 필드는 `actorId`, `requestId`, `ipAddress`, `userAgent`, `entityType`, `entityId`, `action`, `beforeValue`, `afterValue`, `reason`, `idempotencyKey`, `createdAt`이다. 월마감 감사 증적은 `/operations/audit-integrity-report`의 hash chain tail hash 또는 외부 WORM/감사 저장소 보관 증적으로 확인한다.
